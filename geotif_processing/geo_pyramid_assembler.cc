...
int main( int argc, char *argv[] ) 
{
  ...
  //float foreMaxMPP=assemblerParams.foreMaxPPD;
  ...
  if ((mode.compare("DEM")==0)){
    ...
    int overwriteFlag = 1;
    vector<vector<string> > assembledDEMTileFilenameArray = assembleGeoData(foreFilename, backFilename,
  ...
  if ((mode.compare("DRG")==0)){
    //this should be set from the config or command line
    int overwriteFlag = 1;
    vector<vector<string> > assembledDRGTileFilenameArray = assembleGeoData(foreDRGFilename, backDRGFilename,
									    assemblerParams, "DRG", resDir, overwriteFlag);
  }
  
  if (mode.compare("DEM_DRG")==0){
    int overwriteFlag = 1;
    vector<struct TerrainFilename>assembledTileFilenameArray =  pyrAssembleGeoData(foreFilename, backFilename,
